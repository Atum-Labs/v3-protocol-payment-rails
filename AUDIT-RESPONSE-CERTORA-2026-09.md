...
The wrap is a single hash: `keeperDigest(d) == keccak256(0x1901 ‖ domainSeparator ‖ d)`. The Permit2 digest is domain-separated as-is rather than re-hashed under an Atum struct type, because it is already a unique commitment to the whole authorisation and the domain separator is what carries `address(this)` and `chainid` — a struct hash in between would bind nothing further while adding a constant the keeper has to know. The derivation is therefore reproducible off-chain from the module's ERC-5267 `eip712Domain()` alone, which `test_KeeperDigest_IsThePermit2DigestUnderTheModuleDomain` pins.
